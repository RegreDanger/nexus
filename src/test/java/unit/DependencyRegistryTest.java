...
class DependencyRegistryTest {
	...
	void shouldCombineMapsWhenRegisteringMultipleTimes() {
		...
		//Both instances must be available
	...
	@Test
	void registeringWithNonClassKeyShouldThrowInvalidRegistryEntryException() {
		DependencyRegistry base = RegistryProvider.getRegistry(DependencyRegistry.class);
		Map<Object, Object> bad = new HashMap<>();
		bad.put("notAClassKey", new SimpleBean("x"));

		InvalidRegistryEntryException ex = assertThrows(InvalidRegistryEntryException.class,
				() -> base.registry(bad),
				"Registering a map with non-Class keys must throw InvalidRegistryEntryException");

		// Optional: ensure the message is helpful
		assertTrue(ex.getMessage().toLowerCase().contains("all keys must be class") ||
				   ex.getMessage().toLowerCase().contains("all keys must be class"),
				   "Exception message should indicate that keys must be Class<?> instances");
	}
