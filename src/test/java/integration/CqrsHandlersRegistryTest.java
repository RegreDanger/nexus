...
class CqrsHandlersRegistryTest {
	...
	void getCQRSHandlerWhenNotRegisteredShouldThrow() {
		RegistryResolutionException ex = assertThrows(RegistryResolutionException.class,
				() -> cqrsHandlersRegistry.getCQRSHandler(DummyHandler.class),
				"Requesting a handler that is not registered should throw RegistryResolutionException.");
		// message should indicate the handler wasn't found and include the simple name of the handler class
		assertTrue(ex.getMessage().contains("CQRS handler not found"), "Exception message should mention handler not found");
		assertTrue(ex.getMessage().contains(DummyHandler.class.getSimpleName()),
				"Exception message should include the missing handler simple class name.");
	}
